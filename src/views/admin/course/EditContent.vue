    ...
    <v-card-text>
        <v-container>
            ...
            <v-row justify="center" >
                ...
                <ValidationProvider v-slot="{ errors }"  name="Subject Name">
                    <v-text-field
                        name="content_name"
                        label="Subject Title"
...
export default {
  ...
  methods: {
    ...
    initialize(){
        this.getContentById(this.$route.params.id)
        .then((response) => {
      ...
      save(){
          this.$refs.userForm.validate().then((success) => {
            ...
            this.allerror ='';
            const data = {id: this.$route.params.id , subject_name: this.subject_name , contents: this.contents};
