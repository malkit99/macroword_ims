              ...
              <v-row>
                <v-col cols="12" sm="6" md="6">
                  <v-list-item-avatar tile size="200" color="grey">
                    <v-img :src="imageURL ? imageURL : dist.discount_image"></v-img>
                  </v-list-item-avatar>
                  <ValidationProvider
...
initialize() {
    ...
    save() {
      this.$refs.discountForm.validate().then(success => {
        ...
        axios.post(`api/discount/update/${this.$route.params.id}` , data)
          .then(response => {
            this.removeloading({ show: false });   
            this.$router.push({ name: "discount-home" });
            this.addNotification({
              show: true,
              text: "Course Discount Update Successfully"
            });
          })
          .catch(error => {
