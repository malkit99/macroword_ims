              ...
              <v-col cols="12" sm="6" md="8">
                <ValidationProvider v-slot="{ errors }" name="Name" rules="required|min:3|max:30" >
                  ...
                  <ValidationProvider v-slot="{ errors }"  name="Mobile Number" rules="required|max:15">
                    <vue-tel-input v-model="user.mobile"  v-bind="bindProps" @onInput="onInput"></vue-tel-input>
                  ...
                  <v-row>
                    <v-col cols="12">
                      <v-list-item-avatar tile size="200" color="grey">
                        <v-img :src="imageURL ? imageURL : user.profile_image"></v-img>
                      </v-list-item-avatar>
                    </v-col>
                    <v-col cols="12">
                      <ValidationProvider
...
computed: {
  ...
  methods: {
    ...mapActions({
      ...
      userById:'register/userById',
    }),
        initialize() {
      this.userById(this.$route.params.id).then(response => {
        this.user = response.data.data;
      })
      },
     onInput({ number, isValid, country }) {
            this.user.mobile = number.e164
      },
  ...
  onFilePicked(event){   
    ...
    userSave() {
      this.$refs.userForm.validate().then((success) => {
        ...
        data.append('roles', this.user.roles);
        this.addLoading({ show: true , text: "Please Wait Registration Submit ..." });
        axios.post(`api/register/update/${this.$route.params.id}`, data)
        .then((response) => {
