<template>
  <v-card>
    <v-toolbar color="primary" dark>
      <v-card-title>Edit Job Opportunity</v-card-title>
      <v-spacer></v-spacer>
              ...
              <v-row>
                <v-col cols="12" sm="6" md="6">
                  <v-list-item-avatar tile size="200" color="grey">
                    <v-img :src="imageURL ? imageURL : job.job_image"></v-img>
                  </v-list-item-avatar>
                  <ValidationProvider
  ...
  methods: {
    ...
    save() {
      this.$refs.jobForm.validate().then(success => {
        ...
        axios.post(`api/jobOpportunity/update/${this.$route.params.id}`, data)
          .then(response => {
